---
name: track-japanese-tech-writing
description: 日本語の技術記事、書籍原稿、解説文を執筆・推敲するときに使う。整形、論証、概念の導入順序、翻訳調の比喩や冗長な表現の修正を扱う。
---
...
- Neovim などでも本文を追いやすいように、句点で改行し、一文ごとの行を基本にする。
  長い文は、節の境目や列挙の区切りなど、意味のまとまりが切れる位置でも改行する。
  固定の文字数で機械的に折らず、語句、インラインコード、リンク記法の途中は分断しない。
  段落の区切りは空行で示し、通常の改行には Markdown の強制改行用の末尾スペースや `<br>` を付けない。
- コード、差分、ログ、設定ファイルの断片はコードブロックで示す。
- 用語の由来や定式化の名称など、本筋から一段外れる補足は、本文に並べず脚注（`[^ラベル]`）に降ろす。
- 定義や分類の列挙は箇条書きで示してよい。
- 用語を散文中で初めて定義・導入するときは、その語を太字にする。すでに導入した語を話題として指すとき、引用、通称には「」を使う。用語を列挙する箇条書きには、下記のラベル規則を適用する。
- ダッシュ（em ダッシュ `—`、horizontal bar `―`、いわゆる2倍ダッシュ「——」）を日本語の地の文・見出しで使わない。同格・補足の挿入（「A——挿入——B」）は半角括弧 `()` に、言い換え・敷衍（「A——B」）は句点で二文に分けるか読点でつなぐ。範囲を示す en ダッシュ `–` や英語の複合語（`Curry–Howard` など）、コードブロック・書誌情報は対象外。
- 括弧は半角の `()` を使い、全角の `（）` を地の文で使わない。編集時に入力する記号を揃えるための表記規則である。コードブロック・インラインコードは対象外。
- コロンは半角の `: ` を使い、全角の `：` を地の文で使わない。括弧と同じく表記を揃える。行末や閉じ括弧の直前ではスペースを置かない。コードブロック・インラインコードは対象外。
...
- 用語とその定義を並べる箇条書きは、区切り線や太字ラベルを使わず、半角コロンで「用語: 説明」と書く。項目名を太字にすると、機械的に生成した列挙に見える。
...
- 段落間の論理関係が文の内容だけでは伝わらないときは、接続表現で明示する（「であれば」「実際」「しかし」「この例自体からも」）。段落ごとに接続詞を足す必要はない。
- 論証は一方向に進める。結論を導く箇所では、反論と疑念を処理してから結論を置く。
  レポートや要約は結論を先に示し、本文で根拠や適用条件を説明してよい。説明を加えず結論だけを繰り返さない。
...
- 二つの対象を対比して定義するときは、述語の型を揃えて違いを読み取れるようにする。
  既出の説明への参照は文頭に置き、定義を「〜と説明した」の中に埋め込まない。
  続く文の主語が同じで、対比する他方と取り違えない場合は、対象を名乗り直さず説明を続ける。
...
- 読者が新たに覚える前提は、直前までの説明から一段ずつ増やす。
  文章を削ったら残る言及も確認し、使われなくなった主題や参照を残さない。
  読み方の許可や断りは、それだけで伝わるなら直接書く。
...
- 概念の初出では、まず何の一種なのか (データ構造、手順、制約など) を示してから、その性質や仕組みを説明する。
  初めて読む人が戻らずに理解できる順序を選び、定義までの文数は固定しない。
- 新しい例や場面を追加して、読者が保持すべき文脈が増えるときは、前の例と何が違うのか、なぜもう一つ必要なのかを前置きして納得感を与える。
- 図、表、コードを併用するときは、それぞれが示す内容を区別する。
  別の例を使ってもよいが、「この例」などの指示語がどの提示を指すか明らかにし、
  一方で確認した性質を他方へ無条件に当てはめない。
- 方式を置き換える説明では、「Aの代わりにB」のように、やめる方式から採る方式へ順に書く。
  どちらを採用するのかを読み返させない。
- 章冒頭や節の導入では、これから例で扱う内容に関係しない過剰な詳細を詰め込まない。
- 例の節の中でも、**その節の問い・帰結に関係しない**過剰な詳細だけを省く。議論に必要な具体は残す。省略の典型は、エージェント報告の装飾的精度（時刻、HTTP ステータス、カバレッジ率など）や、後で参照しない固有名詞。
...
- 本文中の太字強調を多用しない。誤読を防ぐ否定や節の帰結など、論理の要所に限り、一節に一、二箇所を目安に使う（導入部でも可）。初出の用語を示す太字とは区別する。それ以外は文の順序と構造で際立たせる。
...
本文で定義した術語を議論に使うのはよい。空虚な装飾として使うのが問題である。
...
悪い例: 「多角的に分析すると、重要なのは、入力元によって検証の順序を変えることである」。
良い例: 「入力元によって検証の順序を変える」。

## 翻訳調の比喩と擬人化

比喩のつもりがない文も、英語の言い回しや口頭説明をそのまま移していないか点検する。
対象の動作、確認できる事実、効果の内容を具体的に書く。

- 英語慣用句の直訳: 「情報がこの層に住んでいる」は「この層に情報が含まれる」、
  「問いが開かれている」は「問いは未解決である」とする。
  「選好を運ぶ」も、実際の関係に応じて「選好を表す」「選好が反映される」と書く。
- 抽象物の擬人化: 「モデルが文体を知っている」は「出力の文体が一貫している」など、観察した事実に直す。
  「データが語る」は「データから分かる」とする。
  修正対象は意思や理解を仮定した説明であり、無生物主語を一律に避ける必要はない。
- 話し言葉による評価: 「この手法が効く」「筋がいい」で済ませず、
  何がどう変わるのか、どの条件や根拠から見込みがあるのかを書く。
  測っていない効果や数値を補って具体化しない。

述語を字義どおりに読むと別の動作を想像する場合は、主体と対象を明示して言い直す。
言い直しても内容を特定できなければ、その文を削るか、根拠を確認して書き直す。
定着した専門用語や実際の動作まで、単語だけで機械的に置き換えない。
...
- 同じ主張を言い換えて繰り返さない。
  ただし、同じ題材でも条件や限定など新しい命題を足す文は残す。
  並列項目の位置づけを揃える語の反復も、主張の重複とは区別する。
- 隣接する節が同じことを別の角度で述べているなら、役割が重複している。片方に吸収して一つの節にまとめる。
- 場面を描写した直後に、その内容を要約し直さない。意味づけの一文（「このような作業は、指示さえ渡せば任せられる」など）だけを置く。
- 同じ論理的役割を持つ並列の事実は、対象や条件を取り違えずに読めるなら一文にまとめる。その事実群の論理的地位は文頭の語で示す（「当然、経理部の月次処理も顧客の支払いも〜」）。
- 冗長さを文字数や文数だけで判断しない。
  短くするのは、動機、対象、操作、何が同じ対象を指すか、因果、条件を順に読んで理解できる場合に限る。
  必要な前提を読者の推測に任せる圧縮はせず、説明や反復を残す。
...
- 「有効な対策である」だけで済ませず、本文内の根拠に沿って効果と条件を書く。「有効」を「必須」に変えるなど、語調の修正で主張を強めない。不確実性・可能性・仮定・読者の疑念を表す述語は保持する。
...
- 通読する読み物では、見出しを決め台詞にして本文の導出を先取りしない。レポートでは、必要な判断を拾いやすくするために結論を示す見出しも使える。
- 節で扱っている対象を指す名詞句でもよい。
