...
- 括弧は半角の `()` を使い、全角の `（）` を地の文で使わない。vim の `%` や `f(` といったモーションが全角括弧をペアとして扱えず、編集の妨げになるためである。コードブロック・インラインコードは対象外。
- コロンは半角の `: ` を使い、全角の `：` を地の文で使わない。理由は括弧と同じで、vim の `f:` や `t:` といったモーションが全角コロンに届かないためである。行末や閉じ括弧の直前ではスペースを置かない。コードブロック・インラインコードは対象外。
