---
name: track-tool
description: track ノートに埋め込む HTML ツールを作成・修正するときに使う。単一ファイルと sandbox iframe の制約を扱う。
---
...
埋め込む内容のデザイン（色・余白・階層の考え方）は指示が無く、track のソースが手元にあれば、その `docs/spec/design.md` を参考にする。
ソースがない環境では下記の骨組みを使う。
...
2. 見た目の土台は Pico.css v2 に任せる（指示が無ければ）。生の色を直に書かない。埋め込む内容のデザインは上記の骨組みを基準にし、ツールの primary は `--pico-primary` 系を `--mark` に上書きする。
