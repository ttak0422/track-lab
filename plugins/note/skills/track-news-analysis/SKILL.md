...
description: 一つの時事事象を複数の視点で調査し、出典付きの分析ノートとして track に保存するときに使う。継続観測は track-watch が担う。
---
...
保存先は共通の実行環境に従い、ユーザーの vault 指定と通常の設定を引き継ぐ。
...
引き継ぐ事実ではない。事象の説明に必要なレンズを選ぶ。次の組み合わせを出発点に、関係のない観点は省く。
...
1. Sweep: 各レンズを対象に合う言語で調べ、主張、日付、出典 URL、確信度を記録する。引用には話者・発言日時・媒体を、数値系列には日付・値・単位を添える。
...
対象期間とタイムゾーン、レンズ、調査上限を開始時に記録する。各段階の結果と失敗を作業メモへ残し、再開時は成功済み結果を使って失敗・未確認部分だけ進める。
...
返り値の `execution` と完了ごとのログには成功・失敗・未着手が含まれる。これらは調査の全件成功や vault への保存完了を表さない。スクリプト自体に自動再開はないため、部分失敗からは個別の段階を再実行する。

### 3. 分析 note を書く

対象事象・期間・入力版・設定が同じ既存の分析 note と作業メモを検索し、本文を確認する。再開ならその note の未完了部分を更新する。
初回だけテンプレートから作成する: `track new --title "<YYYYMMDD> <event question>" --template analysis --tag analysis`。
分析 note は事象時点のスナップショットであり、日付付きタイトルと履歴が本体である。`report` タグは付けない。report の契約（無日期タイトル、時系列を積まない）はこの note 種には適用されない。
調査した素材で埋める。出典と検証結果を保ち、図表や書式は事象とユーザー指定に合わせる。
...
- note のタイトル、前提の判定、未検証のまま残ったものをユーザーに伝える。note の再貼り付けではなく、
  短い一段落で。
- 保存・リンク解決・必須情報と、主張の照合・図の検証を別々に記録する。未実行の検証は未実行とし、途中失敗を全件成功と報告しない。
