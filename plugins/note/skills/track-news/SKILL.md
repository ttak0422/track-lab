...
- 自動生成記事とは agent が生成した調査レポート・ハイライト取り込み・集計ノートを指す。人間の手書きノートと区別するために共通タグを付ける（既定は `#auto`。vault ごとに `TRACK_NEWS_TAG` で上書き可）。
- newsダイジェストとは生成記事を「today's top」として番組風に並べたリストを指す。journal の `## News` 見出しの下に置く。各項目は `[[タイトル]]` へのリンク＋一行の襟（description）。
