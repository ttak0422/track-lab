...
description: 説明的な文章に緩急を設計するための規範。緩急を装飾ではなく認知モードの切替（観察→逡巡→断定→再観察）と未回収の緊張の管理として扱い、文の拍、段落の密度波形、図と導線の置き方、緩みと駄文の判別、執筆後の機械的な点検手順を定める。track ボールトで人間が最初から最後まで読まれるページ、すなわち explainer ノートを書くとき、または「密度はあるが平坦でおもしろくない」文章を診断・修正するときに使用する。report ノートは読まれない前提なので対象外。
---
...
- 図（mermaid の flowchart や mindmap、viewspec のチャート）は「具体に寄る」極端な形である。図を置いたら直後に一段引いた段落で意味づけする。図と本文が同じ道順を二度たどらせるなら、どちらか一方に統合する。
...
4. **拍の点検**：長い断定文が3つ以上連続している箇所を探し、短い足場か停止、または逡巡を挿む。図の直後に一段引いた段落があるかも、ここで見る。
