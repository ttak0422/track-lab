...
- `track` はシェルで実行する。実行ファイルはセッションの冒頭で 1 回だけ解決し、以後それを使い続ける（通常は `PATH` 上の `track`、track のソースリポジトリ内では `go run ./cmd/track`。両者は別ビルドを指すことがあるため途中で切り替えない）。出力は機械可読を優先する。通常は JSON、`track export` は Markdown を返す。人間向けと JSON を選べる CLI では `--json` を付ける。終了コードと出力を確認し、失敗したら正確なエラーを報告して止まる。別の実行ファイルへ切り替えたり、サブコマンドやフラグを記憶から推測したりしない。
