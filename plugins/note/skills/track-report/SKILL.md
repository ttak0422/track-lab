...
2. **冒頭に現時点の結論を置く。** 見出しを挟まず、タイトル直下から結論を2〜5行で書く。ここだけ
...
5. **出典はすべて脚注に明示する。** 本文では `[^1]` のように参照し、末尾の脚注定義に URL と取得日を
   書く（`[^1]: https://…（取得 2026-08-22）`）。GFM の `[^label]`＋定義のみを使う。インライン脚注
...
改訂するときは `japanese-report-readability` の削除パスを通す。特に時系列の痕跡（「当初」「まず」
「実は」「追記」）が残っていないかを見る。読者に新旧の取捨選択をさせてはならない。
