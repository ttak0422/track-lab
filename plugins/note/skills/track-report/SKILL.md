---
name: track-report
description: 調査結果を track のレポートノートに保存・改訂するときに使う。通常の実装依頼や短い回答は対象外。
---

# Track Report

CLI を使う前に[実行環境](../track/references/runtime.md)を読む。
資料と生成物の記録は[出典と時点の契約](../track/references/knowledge-intake.md)に従う。
...
調査結果の保存を依頼された場合、または継続中の track 記録作業で共有すべき発見を得た場合に書く。対象となる調査の例を挙げる。
...
通常は次の場合に新規レポートを作らない。ユーザーが保存を明示した場合は、その指定に従う。

- 答えが1回のルックアップ、1ファイル、1コマンドで得られた場合。それは返答であって発見ではない。
- ユーザーが答えではなく変更を求めた場合。それはレポートではなく作業依頼として扱う。
...
保存が依頼範囲に含まれるなら、作成・検証まで続ける。回答だけの依頼を保存の確認待ちで止めない。

## ノートの契約

既定の書式は次のとおり。ユーザー指定の構成や既存ノートの契約がある場合はそちらに合わせ、出典と未確定事項を保つ。
...
2. **冒頭に現時点の結論を置く。** `from [[<project>]]` 行の直後、見出しを挟まずに結論を2〜5行で書く。ここだけ
   読めば現在の状態が分かる。本文の変更で結論が変わる場合は、ここも更新する。
3. **節構成は自由。** 主題の論理に従う。Question/Answer/Evidence のような定型は使わない。
4. **`## 未決` は末尾に固定する。** 未解決の問いを問いの形で列挙する。次のセッションの入り口に
   なる。解決した項目は消し、未決事項がなければ節を省く。
5. **出典はすべて脚注に明示する。** 本文では `[^1]` のように参照し、末尾の脚注定義に
   `[タイトル](URL)` の形式で書く（`[^1]: [Title](https://…)`）。URL だけを書いてはならない。GFM の `[^label]`＋定義のみを使う。インライン脚注
   `^[text]` は track-markdown が生成禁止にしている。脚注に実際に読んだ版・位置・取得日時を添え、保存版があればそこへもリンクする。
6. 入力に使った版、生成日時、処理方法と設定の識別を残す。不明値は不明とし、原文の引用と生成した解釈を区別する。
...
- 出典は本文中の脚注参照（`[^1]` 等）から末尾の脚注定義へ解決する。`[タイトル](URL)` の形式で書く。
...
重要な主張の脚注から、読んだ資料の版と根拠本文へ戻れるか確認する。保存成功と内容の検証結果は分けて報告する。
