---
name: track-report
description: 調査の結果をレポートノートとして track ボールトに保存する。ユーザーが調査・リサーチ・分析・比較を依頼したとき、あるいは何かがそのように振る舞う理由を突き止めるよう求められたときに使う。答えをノートとして書き留め、発見がセッションの流れで消えずに残るようにする。やるべき仕事を記録する task プラグインの track-project-intake と対になる。
---
...
- ユーザーが答えではなく変更を求めた場合。それは task プラグインの `track-project-intake`（記録する）または
...
4. **`## 未決` は末尾に固定する。** 未解決の問いを問いの形で列挙する。次のセッションの入り口に
   なる。解決した項目は消す。
5. **出典はすべて脚注に明示する。** 本文では `[^1]` のように参照し、末尾の脚注定義に URL を
   書く（`[^1]: https://…`）。GFM の `[^label]`＋定義のみを使う。インライン脚注
...
- 出典は本文中の脚注参照（`[^1]` 等）から末尾の脚注定義へ解決する。URL を書く。
