...
description: track の完了項目、期限切れ、WAITING、終わった議論を棚卸しするときに使う。未処理項目の実装は track-task-runner が担う。
---
...
確認できた項目は、[runner の完了項目移動手順](../track-task-runner/SKILL.md#5-完了項目を-worklog-ノートへ移す)に従い、`refile --line` で worklog へ移す。
既存の完了日と証拠を保ち、新たな実装・コミット・状態の付け直しは行わない。
移動先の保存を確認するまで元の項目を除去しない。未完了の子を含む親は残す。
