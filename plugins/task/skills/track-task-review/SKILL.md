...
確認できた項目は [runner の worklog 移動手順](../track-task-runner/SKILL.md#5-完了項目を-worklog-ノートへ移す) に従う。実装・コミットは行わず、元の完了日と証拠を維持する。未完了の子を含む親は移さない。

## 3. 期限と待ちを見直す

期限切れは理由と現在の見込みを確認する。ユーザーの指定や合意済み計画に新しい期限がある場合だけ `task date` で変更し、根拠がなければ元の期限を保って報告する。着手条件が外にあるなら `WAITING` へ置き、待ち先を `(YYYY-MM-DD: <待ち先> 待ち)` と追記する。
...
ユーザーの指定や依存関係、合意済みの期限を根拠に優先度を見直す。根拠のない `[#A]` の追加・削除はしない。優先度の編集は共通規約に従う。
...
`export` で移動した項目だけが元ノートから除かれ、要確認の `[x]` と未完了項目が残っていること、`tasks --overdue` が期待通りになったことを確認する。畳んだ節はアーカイブ先のノートに `[[…]]` 付きで入っていることを確かめる。
